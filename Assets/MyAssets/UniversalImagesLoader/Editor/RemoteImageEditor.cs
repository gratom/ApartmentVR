...
[CustomPropertyDrawer(typeof(RemoteImage))]
public class RemoteImageEditor : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (isOpen)
        {
            ...
            if (property.FindPropertyRelative("_RemoteItemInstance").objectReferenceValue != null)
            {
                Texture2D tempTexture = (Texture2D)property.FindPropertyRelative("_RemoteItemInstance").objectReferenceValue;
                EditorGUI.DrawPreviewTexture(new Rect(position.x + Offset, position.y + (StandartElementHeight + Offset) * 4, 
                    StandartElementHeight * 5f * ((float)tempTexture.width / (float)tempTexture.height), 
                    StandartElementHeight * 5f), tempTexture);
            }
        }

        if (property.FindPropertyRelative("_IsReady").boolValue)
        {
            EditorGUI.LabelField(isReadyRect, "Remote image " + property.FindPropertyRelative("_Name").stringValue + "<color=#005000>(READY)</color>", style);
        }
        else
        {
            EditorGUI.LabelField(isReadyRect, "Remote image " + property.FindPropertyRelative("_Name").stringValue + "<color=#c05050>(NOT READY)</color>", style);
        }

        if (GUI.Button(openCloseButtonRect, (isOpen) ? ("-") : ("+")))
        {
            isOpen = !isOpen;
            currentHeight = isOpen ? (StandartElementHeight + Offset) * ((property.FindPropertyRelative("_RemoteItemInstance").objectReferenceValue == null) ?(4):(9)) : StandartElementHeight + Offset;
        }
